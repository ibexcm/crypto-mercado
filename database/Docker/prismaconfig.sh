...
sed -i "s/__MANAGEMENT_API_SECRET__/$(_R=$(echo $SECRET | ./jq -r '.PRISMA_MANAGEMENT_API_SECRET'); [ $_R == null ] || [ -z $_R ] && echo $MANAGEMENT_API_SECRET || echo $_R)/g" prismaconfig.yml
sed -i "s/__PORT__/$(_R=$(echo $SECRET | ./jq -r '.PRISMA_PORT'); [ $_R == null ] || [ -z $_R ] && echo $PRISMA_PORT || echo $_R)/g" prismaconfig.yml
sed -i "s/__DATABASE_CONNECTOR__/$(_R=$(echo $SECRET | ./jq -r '.CONNECTOR'); [ $_R == null ] || [ -z $_R ] && echo $CONNECTOR || echo $_R)/g" prismaconfig.yml 
sed -i "s/__ENABLE_DB_MIGRATIONS__/$(_R=$(echo $SECRET | ./jq -r '.MIGRATION'); [ $_R == null ] || [ -z $_R ] && echo $MIGRATION || echo $_R)/g" prismaconfig.yml 
